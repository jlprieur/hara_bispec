...
C XCR: Reference spectrum (list), phase factor  (not known for real observ.)
	...
	INTEGER IXY,KLM,IFERMAX,NBCOUV,NX,NY,NBX,NBY
        REAL SIGM(NBMAX),SIG_MAX,X,BMASK
...
52	FORMAT(' Program inv_bispec Version 15-04-92 ',
     1	A,/,' Option = ',I3,/,
...
29	FORMAT(' NBETA (used here):',I4,' Corresponding NGAMMA:',I5)
...
C ECART DE PHASE ANGULAIRE POINT PAR POINT EXPRIME EN RADIAN
	  DO 10 NB = 0,NBETA
	    ...
	    CC = XC(NB)
	    X(NB,0) = BMASK(NB)*(XX - ATAN2(IMAG(CC),REAL(CC)))
10	  CONTINUE
...
C ALPHA_0 EN X(.,0)
	DO 1 NB=0,NBETA
	    CC=XC(NB)
	    X(NB,0) = BMASK(NB)*ATAN2( IMAG(CC), REAL(CC) )
1	CONTINUE
...
C FACTEUR DE PHASE INITIAL CORRESPONDANT
	DO 2 NB=0,NBETA
	  ALPHA=X(NB,0)
          COS1=BMASK(NB)*COS(ALPHA)
          SIN1=BMASK(NB)*SIN(ALPHA)
